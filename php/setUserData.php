<?php
    ...
    if ($dataFromJS['request'] == "setUserData") {
        $userId = $dataFromJS['userId'];
        $userData = MakeValue($dataFromJS['userDataToPost']);
        print_r($dataFromJS['userDataToPost']);

        // Delete smiles from text.
        ...
        $query = "UPDATE users_info SET user_data = '{$userDataWithSpecSymbols}' WHERE id = '{$userId}'";
        // $query = 'UPDATE users_info SET user_data='.$city.' WHERE id='.$userId;
        ...
        echo "Error. Request doesn't exists";
    }
